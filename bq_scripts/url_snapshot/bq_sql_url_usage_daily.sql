...
Begin
     ...
     create temp table t_urlsnap_prototype_lastday_1
      as
       select
          reg_domain,
       ...
       create temp table t_urlsnap_prototype_lastday_2
      as
       select
          reg_domain,
          unique_visitors,
      ...
      create temp table t_urlsnap_prototype_lastday_3
    as
      select
...
*/
    ...
    create temp table t_urlsnap_prototype_lastday_4
    as
        select
        sum(case 
...
-- Storing Page URLs weekly aggregated dimensions in BigQuery
  create or replace table dta_customers.ua_61222473_1_urlpage_usage_lastday
      OPTIONS (
            description = "Daily url page usage dimensions on daily schedule",
    ...
    as
        select
          ...
          cast(uw4.traffic_organic as string) as organic,
          cast(uw4.traffic_referral as string) as referral,
          cast(uw4.traffic_other as string) as other,

          cast(uw3.mobile_count as string) as mobile,
          cast(uw3.desktop_count as string) as desktop,
          cast(uw3.tablet_count as string) as tablet,

          cast(round(uw2.time_on_page,1) as string) as time_on_page,

          cast(uw1.newUsers as string) as new_users,
          cast(uw1.returningUsers as string) as returning_users,
          cast(uw1.ratio as string) as ratio
          from t_urlsnap_prototype_lastday_4 uw4
          inner join t_urlsnap_prototype_lastday_3 uw3
            on uw4.reg_domain = uw3.reg_domain and uw4.pagepath = uw3.pagepath
          inner join t_urlsnap_prototype_lastday_2 uw2
            on uw4.reg_domain = uw2.reg_domain and uw4.pagepath = uw2.pagepath
          inner join t_urlsnap_prototype_lastday_1 uw1
            on uw4.reg_domain = uw1.reg_domain and uw4.pagepath = uw1.pagepath;

DROP TABLE IF EXISTS t_urlsnap_prototype_lastday_1;
DROP TABLE IF EXISTS t_urlsnap_prototype_lastday_2;
DROP TABLE IF EXISTS t_urlsnap_prototype_lastday_3;
DROP TABLE IF EXISTS t_urlsnap_prototype_lastday_4;
End;
...
-- Storing Page URLs weekly aggregated dimensions in BigQuery
  create or replace table dta_customers.ua_61222473_15_urlpage_usage_lastday
      OPTIONS (
            description = "Daily url page usage dimensions on daily schedule",
            expiration_timestamp = TIMESTAMP_ADD(current_timestamp, INTERVAL 24 HOUR)
  )
...
-- Property 3
-- Page URL usage of property UA-61222473-13 - domainname.gov.au
Begin
...
-- Storing Page URLs weekly aggregated dimensions in BigQuery
  create or replace table dta_customers.ua_61222473_13_urlpage_usage_lastday
      OPTIONS (
            description = "Daily url page usage dimensions on daily schedule",
            expiration_timestamp = TIMESTAMP_ADD(current_timestamp, INTERVAL 24 HOUR)
  )
...
-- Property 4
-- Page URL usage of property UA-61222473-33 - stylemanual.gov.au
Begin
...
-- Storing Page URLs weekly aggregated dimensions in BigQuery
  create or replace table dta_customers.ua_61222473_33_urlpage_usage_lastday
      OPTIONS (
            description = "Daily url page usage dimensions on daily schedule",
            expiration_timestamp = TIMESTAMP_ADD(current_timestamp, INTERVAL 24 HOUR)
  )
